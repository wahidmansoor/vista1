...
export class ProtocolMatcher {
  ...
  private DATASET_DISCLAIMER = "Note: Local protocol dataset is incomplete and requires expert oncology review.";

  constructor(protocols: TreatmentProtocol[]) {
  ...
  public findMatches(
    ...
    return this.protocols
      ...
      .sort((a, b) => {
        ...
        if (a.protocol.preferredOption && !b.protocol.preferredOption && a.isEligible) return -1;
        if (!a.protocol.preferredOption && b.protocol.preferredOption && b.isEligible) return 1;
  ...
  private evaluateProtocol(
    protocol: TreatmentProtocol,
    disease: DiseaseStatus,
    ecog: number,
    currentLine: number,
    history: TreatmentLine[]
  ): ProtocolMatch {
    ...
    if (protocol.histology && protocol.histology.length > 0) {
      if (!disease.histology) {
        ...
        matchedFactors.push(`Histology: ${disease.histology}`);
      }
    }

    // 0. Preferred Option Bonus
    if (protocol.preferredOption) matchScore += 10;
    ...
    if (biomarkerResult.missingHardData) hasMissingData = true;
    if (biomarkerResult.isMismatched) hasStrictMismatch = true;
    ...
    matchedFactors.push(...biomarkerResult.matchedFactors);
    ...
    if (ecog > protocol.maxEcog) {
      ...
      matchedFactors.push(`ECOG: ${ecog}`);
    }

    // 4. Stage matching
    if (protocol.stageEligibility && !(protocol.stageEligibility as string[]).includes('Any')) {
      if (!disease.stageAtDiagnosis) {
        hasMissingData = true;
        warnings.push("Missing Data: Stage not specified");
      } else if (!(protocol.stageEligibility as string[]).includes(disease.stageAtDiagnosis)) {
        hasStrictMismatch = true;
        warnings.push(`Mismatch: Protocol stage ${protocol.stageEligibility.join('/')} vs Patient ${disease.stageAtDiagnosis}`);
      } else {
        matchedFactors.push(`Stage: ${disease.stageAtDiagnosis}`);
      }
    }
    // 4. Stage Eligibility 
    const stageArray = protocol.stageEligibility as string[];
    ...
    let status: ProtocolMatch['status'] = 'Not eligible';
    ...
    return {
      ...
      status,
      matchedFactors: status !== 'Not eligible' ? matchedFactors : []
    };
  }

  private checkBiomarkers(protocol: TreatmentProtocol, disease: DiseaseStatus) {
    const warnings: string[] = [];
    const matchedFactors: string[] = [];
    ...
    const results = protocol.requiredBiomarkers.map(req => {
      ...
      if (!patient || !patient.status || patient.status === 'Unknown') {
        return { satisfied: false, missing: true, hard: req.urgency === 'Hard', name: req.name };
      }
      return { satisfied: patient.status === req.status, missing: false, name: req.name, status: patient.status };
    });

    if (protocol.biomarkerLogic === 'ALL_OF') {
      if (results.some(r => r.missing && r.hard)) missingHardData = true;
      if (results.some(r => !r.satisfied && !r.missing)) isMismatched = true;
      
      // If all satisfied
      if (results.every(r => r.satisfied)) {
        results.forEach(r => matchedFactors.push(`${r.name}: ${r.status}`));
      }
    } else { // ANY_OF
      const anySatisfied = results.some(r => r.satisfied);
      if (!anySatisfied) {
        // Fix: If there are NO required biomarkers, it counts as satisfied for ANY_OF
        if (protocol.requiredBiomarkers.length === 0) {
           return { missingHardData: false, isMismatched: false, hasExcludedMarkers: false, warnings: [], matchedFactors: [] };
        }
        if (results.every(r => r.missing && r.hard)) missingHardData = true;
        else isMismatched = true;
      } else {
        results.filter(r => r.satisfied).forEach(r => matchedFactors.push(`${r.name}: ${r.status}`));
      }
    }
  ...
  private checkPriorAgents(protocol: TreatmentProtocol, history: TreatmentLine[]): string[] {
  ...
  private createEmptyMatch(protocol: TreatmentProtocol): ProtocolMatch {
    return {
      protocol,
      isEligible: false,
      matchScore: 0,
